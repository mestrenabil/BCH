...
- **JSON Backup**: `GET /api/backup` (مسؤول فقط) — يصدّر الكيانات الرئيسية، و`POST /api/backup` يستعيد نسخة 3.x بالدمج داخل transaction دون حذف البيانات الحالية ويسجل العملية في `ActivityLog`.
- **التخزين الخارجي**: `POST /api/backup/remote` (مسؤول فقط) يرفع النسخة إلى Google Drive أو WebDAV عندما تكون أسرار الربط مضبوطة في بيئة الخادم؛ لا تُعاد هذه الأسرار إلى المتصفح.
