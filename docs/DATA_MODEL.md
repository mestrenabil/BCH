...
## 15. إحصائيات استعمال المنصة

يحفظ `PlatformVisit` أحداث الاستعمال بأربعة أنواع: الواجهة العمومية، صفحات التبليغ، أقسام المنصة الداخلية، وتسجيل الدخول. البيانات المتاحة للعرض محصورة في المسؤول العام عبر:

- `POST /api/platform-analytics/track` لتسجيل زيارة مع منع التكرار خلال 30 دقيقة.
- `GET /api/platform-analytics?days=30` لعرض المجاميع والاتجاهات والترتيب.

لا يحفظ الجدول عنوان IP الخام أو الإحداثيات الدقيقة. يستخدم `ANALYTICS_HASH_SALT` لإنشاء بصمة مجهولة، ولا تُربط الجماعة بالزائر العمومي إلا بعد تحديدها في صفحة التبليغ.
