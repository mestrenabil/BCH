...
type OperationsSummary = {
  generatedAt: string
  daily: { newComplaints: number; publicComplaints: number; scheduledOrders: number; completedToday: number; activeAgents: number }
...
export default function OperationsView() {
  ...
  return <div className="space-y-5 p-4 pb-24 lg:p-6 lg:pb-6" dir="rtl">
    <section className="flex flex-col gap-4 rounded-3xl bg-gradient-to-l from-slate-950 via-indigo-950 to-violet-900 p-5 text-white shadow-lg sm:flex-row sm:items-center sm:justify-between">
      <div><p className="text-sm font-semibold text-violet-200">قراءة مباشرة لعمل هذا اليوم</p><h1 className="mt-1 text-2xl font-black">مركز القيادة اليومي</h1><p className="mt-2 text-sm text-slate-300">ركّز على المهام المتأخرة والقريبة من آخر أجل قبل أن تتحول إلى شكايات غير معالجة.</p></div>
    ...
    <section><h2 className="mb-3 font-black text-slate-900">مؤشرات اليوم</h2><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="بلاغات جديدة" value={summary.daily.newComplaints} color="bg-cyan-50 text-cyan-800" /><Metric label="بلاغات عمومية" value={summary.daily.publicComplaints} color="bg-sky-50 text-sky-800" /><Metric label="أوامر مبرمجة" value={summary.daily.scheduledOrders} color="bg-indigo-50 text-indigo-800" /><Metric label="منجزة اليوم" value={summary.daily.completedToday} color="bg-emerald-50 text-emerald-800" /><Metric label="أعوان نشطون" value={summary.daily.activeAgents} color="bg-teal-50 text-teal-800" /></div></section>
