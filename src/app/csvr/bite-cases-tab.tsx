...
export default function BiteCasesTab({ animals, buildParams, onRefresh }: Props) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const response = await fetch('/api/bite-cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, commune: buildParams().get('commune') || allowedCommunes[0] || '', victimAge: form.victimAge || null, animalId: form.animalId || null, latitude: form.latitude || null, longitude: form.longitude || null, source: 'INTERNAL' }) })
  ...
  return <div className="space-y-4" dir="rtl">
    ...
    <form onSubmit={submit} className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div className="flex items-center gap-2 sm:col-span-2 lg:col-span-4"><input placeholder="خط العرض" value={form.latitude} onChange={(event) => setForm({ ...form, latitude: event.target.value })} className={input} /><input placeholder="خط الطول" value={form.longitude} onChange={(event) => setForm({ ...form, longitude: event.target.value })} className={input} /><LocationPicker latitude={form.latitude} longitude={form.longitude} allowedCommunes={allowedCommunes} label="تحديد موقع الحادث" className="shrink-0" onSelect={({ latitude, longitude }) => setForm({ ...form, latitude: String(latitude), longitude: String(longitude) })} /></div>
