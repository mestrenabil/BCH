...
export default function CentersTab({ animals, buildParams, onRefresh, mapAllowedCommunes }: Props) {
  ...
  const createCenter = async (event: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      const response = await fetch('/api/csvr/centers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...centerForm, commune: buildParams().get('commune') || '' }) })
      const data = await response.json().catch(() => ({}))
      if (!response.ok) { toast.error(data.error || 'تعذر إنشاء المركز'); return }
      toast.success('تم إنشاء المركز')
      setCenterForm({ name: '', type: 'REFUGE', adresse: '', responsible: '', telephone: '', capacity: '', latitude: '', longitude: '', notes: '' })
      await loadCenters()
      onRefresh()
