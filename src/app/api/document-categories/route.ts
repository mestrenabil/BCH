...
export async function GET(request: NextRequest) {
  try {
    ...
    const categories = await db.documentCategory.findMany({ where, orderBy: { label: 'asc' } })
    return NextResponse.json({ categories })
  } catch (error) {
    console.error('GET document categories error:', error)
    return NextResponse.json({ error: 'حدث خطأ أثناء تحميل فئات المستندات' }, { status: 500 })
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('POST document category error:', error)
    if (error instanceof Error && error.message.includes('Unique constraint')) {
      return NextResponse.json({ error: 'هذه الفئة موجودة مسبقاً في الجماعة' }, { status: 409 })
    }
    return NextResponse.json({ error: 'حدث خطأ أثناء إضافة فئة المستند' }, { status: 500 })
  }
}
